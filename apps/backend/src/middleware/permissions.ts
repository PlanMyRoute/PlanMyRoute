 ...
 */
async function getUserRoleInTrip(
  userId: string,
  tripId: number,
): Promise<ExtendedRole> {
...
export function requirePermission(action: TripAction) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Siempre usar el userId del JWT verificado (req.userId),
      // nunca el que envía el cliente, para evitar escalada de privilegios.
      ...
      if (!hasPermission(role, action, tripStatus)) {
        ...
        if (role === "guest") {
          errorMessage =
            "No tienes acceso a este viaje. Solo puedes visualizarlo.";
