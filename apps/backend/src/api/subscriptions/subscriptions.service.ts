...
export async function getIsPremium(userId: string): Promise<boolean> {
  ...
  const notExpired = data.current_period_end
    ...
    : false; // Sin fecha de fin → no es premium
 ...
 */
export async function redeemReferral(refereeId: string, referralCode: string) {
  ...
  if (existingReferral)
    throw new Error("Ya has canjeado un código de invitación anteriormente");

  // 3. Determinar el estatus del Referrer para saber el premio
  const referrerSub = await getSubscription(referrerUser.id);
  ...
  await addPremiumDays(referrerUser.id, daysToReward); // Al que invitó
  await addPremiumDays(refereeId, daysToReward); // Al invitado
 ...
 */
export async function redeemPromoCode(userId: string, codeInput: string) {
  ...
  if (promo.max_uses && (promo.used_count ?? 0) >= promo.max_uses)
    throw new Error("El código se ha agotado");

  // Aplicar días
  await addPremiumDays(userId, promo.duration_days);
 ...
 */
async function addPremiumDays(userId: string, days: number) {
  const sub = await getSubscription(userId);
  const newEndDate = calculateNewEndDate(sub.current_period_end, days);
