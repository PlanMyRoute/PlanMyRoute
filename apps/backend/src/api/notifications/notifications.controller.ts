...
export const createNotification = async (
  ...
  try {
    ...
    if (!payload || !payload.user_receiver_id || !payload.content) {
      res.status(400).json({
        error: "Faltan campos obligatorios: user_receiver_id y content",
      });
      return;
    }
...
export const acceptInvitation = async (
  ...
  try {
    ...
    if (!notification.related_trip_id) {
      res
        .status(400)
        .json({ error: "La notificación no tiene un viaje asociado" });
      return;
    }
    ...
    if (!notification.user_receiver_id) {
      res
        .status(400)
        .json({ error: "La notificación no tiene un usuario receptor" });
      return;
    }

    if (!role) {
      res.status(400).json({ error: 'Falta el campo "role" en el body' });
      return;
    }
    ...
    try {
      ...
      res.status(404).json({ error: "El viaje asociado no existe" });
      return;
    }
    ...
    try {
      ...
      if (
        ...
        res.status(409).json({ error: "El usuario ya pertenece a este viaje" });
        return;
      }
...
export const getNotificationsByTrip = async (
  req: Request,
  res: Response,
): Promise<void> => {
