 ...
 */
export const canUserReviewTrip = async (
  tripId: string,
  userId: string,
  ...
  if (trip.end_date) {
    ...
    if (daysSinceEnd > REVIEW_WINDOW_DAYS) {
      return {
        canReview: false,
        reason: `El plazo de ${REVIEW_WINDOW_DAYS} días para reseñar ha expirado`,
      };
    }
  }
...
export const deleteReview = async (reviewId: string, userId: string) => {
  ...
  // Eliminar
  const { error } = await supabase.from(TABLE_NAME).delete().eq("id", reviewId);
 ...
 */
export const getPublicReviewsFeed = async (
  limit: number = 20,
  offset: number = 0,
) => {
  const { data, error } = await supabase
 ...
 */
export const getSocialReviewsFeed = async (
  userId: string,
  limit: number = 20,
  offset: number = 0,
) => {
  ...
  if (followingError) {
    throw new Error(
      `Error al obtener usuarios seguidos: ${followingError.message}`,
    );
  }
  ...
  const socialNetworkIds = Array.from(
