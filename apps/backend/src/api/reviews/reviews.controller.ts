...
export const createReview = async (
  ...
  try {
    ...
    if (!trip_id || !rating) {
      res.status(400).json({ error: "trip_id y rating son requeridos" });
      return;
    }
    ...
    if (err.message.includes("ya tiene") || err.message.includes("Ya existe"))
      return void res.status(409).json({ error: err.message });
    if (
      err.message.includes("no es viajero") ||
 ...
 */
export const getUserReviewForTrip = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
