...
export async function suggestRefuel(req: Request, res: Response) {
  try {
    const { tripId: tripIdParam } = req.params as Record<string, string>;
    ...
    try {
      ...
      if (err instanceof InsufficientTokensError) {
        return res.status(402).json({
          error:
            "No tienes tokens suficientes para usar el asistente de repostaje.",
    ...
    if (suggestions.length === 0) {
      return res.json({
        suggestions: [],
        message:
          "El vehículo llega a todas las paradas sin necesidad de repostar.",
      });
    }
    ...
    return res
      .status(500)
      .json({ error: "Error al calcular paradas de repostaje" });
  }
}
...
export async function applyRefuelSuggestions(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Error al crear las paradas de repostaje" });
  }
}
