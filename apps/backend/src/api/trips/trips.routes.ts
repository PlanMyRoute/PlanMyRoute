...
router.post(
  `${TRAVELERS_BASE_PATH}/:userId${TRIP_BASE_PATH}`,
  verifyToken,
...
router.patch(
  `${TRAVELERS_BASE_PATH}/:userId${TRIP_BASE_PATH}/:id`,
  verifyToken,
  requireEditor(),
...
router.get(
  `${TRAVELERS_BASE_PATH}${TRIP_BASE_PATH}/:id`,
  verifyToken,
  requirePermission("view_trip"),
  TripController.getTravelersInTrip,
);

// Nueva ruta: Obtener historial de viajes completados de un usuario (debe ir ANTES de la ruta general)
router.get(
  `${TRAVELERS_BASE_PATH}/:userId${TRIP_BASE_PATH}s/history`,
  verifyToken,
  TripController.getUserTripHistory,
);

// Nueva ruta: Obtener viajes de un usuario específico (no requiere permisos)
router.get(
  `${TRAVELERS_BASE_PATH}/:userId${TRIP_BASE_PATH}s`,
  verifyToken,
...
router.delete(
  `${TRAVELERS_BASE_PATH}/:userId${TRIP_BASE_PATH}/:tripId/leave`,
  verifyToken,
  requirePermission("leave_trip"),
  TripController.leaveTrip,
);

// Cambiar rol - requiere permiso change_roles (solo owner)
router.patch(
  `${TRAVELERS_BASE_PATH}/:userId${TRIP_BASE_PATH}/:tripId/role`,
  verifyToken,
  requirePermission("change_roles"),
  TripController.changeUserRole,
);

// Expulsar viajero - requiere permiso remove_travelers (solo owner)
router.delete(
  `${TRAVELERS_BASE_PATH}/:userId${TRIP_BASE_PATH}/:tripId/kick`,
  verifyToken,
  requirePermission("remove_travelers"),
  TripController.removeUserFromTrip,
);

// =============== VEHICLE ROUTES ===============
// Obtener vehículos de un viaje
router.get(
  `${TRIP_BASE_PATH}/:id${VEHICLE_BASE_PATH}s`,
  verifyToken,
  requirePermission("view_trip"),
  TripController.getVehiclesInTrip,
);

// Eliminar vehículo de un viaje (requiere editor o owner)
router.delete(
  `${TRAVELERS_BASE_PATH}/:userId${TRIP_BASE_PATH}/:tripId${VEHICLE_BASE_PATH}/:vehicleId`,
  verifyToken,
  requireEditor(),
  TripController.removeVehicleFromTrip,
);

// =============== TRIP STATUS MANAGEMENT ROUTES ===============
// Responder a notificación de cambio de estado (solo owner puede responder)
router.post(
  `${TRIP_BASE_PATH}/:id/status/respond`,
  verifyToken,
  TripController.respondToTripStatus,
);

// Obtener nivel de acceso del usuario actual en un viaje
router.get(
  `${TRIP_BASE_PATH}/:id/access-level`,
  verifyToken,
  TripController.getAccessLevel,
);
