...
/** Delay entre llamadas a Google Places API para respetar rate limits (ms) */
...
export async function getPlacePrice(
  ...
  try {
    ...
    detailsUrl.searchParams.append(
      "fields",
      "price_level,rating,user_ratings_total,formatted_address,website,opening_hours",
    );
    ...
    if (detailsData.status !== "OK") {
      ...
      return {
        ...
        price_symbol: place.price_level
          ? getPriceSymbol(place.price_level)
          : undefined,
        rating: place.rating,
    ...
    const priceInfo = {
      ...
      price_symbol: details.price_level
        ? getPriceSymbol(details.price_level)
        : undefined,
      estimated_price: details.price_level
        ? getEstimatedPrice(details.price_level)
        : "Precio desconocido",
    ...
    console.error(
      "❌ Error al obtener información de precio del lugar:",
      error,
    );
    return null;
  }
