...
/**
 * Construye la URL de la API de Google Places para obtener una foto
 * @param photoReference Referencia de la foto obtenida de la API
...
/**
 * Extrae la URL de la foto del primer resultado de la respuesta de Google Places
 * @param searchData Respuesta de la API de Places
 * @param queryLabel Etiqueta descriptiva para los logs
 * @returns URL de la foto, o null si no hay resultados o fotos
 */
function extractPhotoFromResults(
  searchData: PlacesSearchResponse,
  queryLabel: string,
): string | null {
...
export async function searchPlacePhoto(
  ...
  try {
    ...
    // 2. Extraer foto del primer resultado
    return extractPhotoFromResults(searchData, name);
  } catch (error) {
 ...
 * @returns URL de la foto del lugar, o null si no se encuentra
 */
export async function searchPlacePhotoByQuery(
  query: string,
): Promise<string | null> {
  try {
    if (!GOOGLE_PLACES_API_KEY) {
...
export async function searchPlacePhotoFoursquare(
  ...
  try {
    if (!FOURSQUARE_API_KEY) {
      ...
      console.warn(
        "📝 Configura FOURSQUARE_API_KEY=tu_api_key en el archivo .env",
      );
      console.warn(
        "🔗 Obtén tu API key en: https://location.foursquare.com/developer/",
      );
      return null;
    }
    ...
    console.log(
      `🔍 Foursquare: Buscando "${name}" en (${coordinates.latitude}, ${coordinates.longitude})`,
    );
    ...
    if (!searchResponse.ok) {
      if (searchResponse.status === 401) {
        console.error(
          `❌ Foursquare API error 401: API Key inválida o no autorizada`,
        );
        console.error(`📝 Verifica tu FOURSQUARE_API_KEY en el archivo .env`);
        console.error(
          `🔗 Regenera tu API key en: https://location.foursquare.com/developer/`,
        );
      } else if (searchResponse.status === 410) {
        console.warn(`⚠️ Foursquare API error 410: Endpoint deprecado`);
        console.warn(
          `📚 Foursquare ha actualizado su API. Usando Google Places como alternativa.`,
        );
      } else if (searchResponse.status === 429) {
        console.error(
          `⚠️ Foursquare API error 429: Límite de llamadas excedido`,
        );
        console.error(
          `📊 Has superado las 50,000 llamadas/mes del plan gratuito`,
        );
      } else {
...
export async function searchPlacePhotoByQueryFoursquare(
  ...
  try {
    ...
    console.log(
      `✅ Foursquare: Foto encontrada para "${query}": ${place.name}`,
    );
    return photoUrl;
  } catch (error) {
    console.error("Error en Foursquare API:", error);
