...
export const sendPushNotification = async (
  ...
  try {
    ...
    if (!isValidExpoPushToken(pushToken)) {
      console.warn(
        "⚠️ [PushNotification] Formato de token Expo inválido:",
        pushToken,
      );
      return { success: false, error: "Formato de token push inválido" };
    }
    ...
    console.log("📲 [PushNotification] Enviando notificación:", {
      to: pushToken,
      title,
    ...
    if (result.data && result.data.length > 0) {
      ...
      if (firstResult.status === "error") {
        console.error("❌ [PushNotification] Error de Expo API:", firstResult);
        return {
          success: false,
          error: firstResult.message || "Error desconocido de Expo API",
        };
      }

      console.log(
        "✅ [PushNotification] Notificación enviada correctamente:",
        firstResult.id,
      );
      return { success: true };
    }

    return { success: false, error: "Sin datos de respuesta de Expo API" };
  } catch (error) {
    console.error("❌ [PushNotification] Error al enviar notificación:", error);
    return {
      success: false,
...
export const sendBatchPushNotifications = async (
  ...
  console.log(
    `📲 [PushNotification] Enviando ${messages.length} notificaciones en lote`,
  );
...
export const formatNotificationForPush = (
  ...
  switch (type) {
    ...
    case "trip_status_check":
      if (
        content.includes("empezar") ||
        content.includes("comenzar") ||
        content.includes("partir")
      ) {
        return {
