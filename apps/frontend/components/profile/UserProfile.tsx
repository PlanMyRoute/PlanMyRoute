...
export default function UserProfile({
  ...
  const {
    ...
    handleDeleteVehicle,
    maxVehicles,
  } = useVehicles(isOwnProfile ? userId : undefined);
  ...
  const { follow, unfollow, isFollowPending, isUnfollowPending } =
    useFollowUser();
  ...
  >(null);
  const { data: followersList, isLoading: followersLoading } = useFollowers(
    followListType === "followers" ? userId : undefined,
  );
  const { data: followingList, isLoading: followingLoading } = useFollowing(
    followListType === "following" ? userId : undefined,
  );
  ...
  const { data: existingReview } = useUserTripReview(
    selectedTripForReview?.id?.toString() || "",
  );
    ...
    try {
      ...
      if (!result.canceled && result.assets?.length > 0) {
        ...
        showAlert({
          ...
          message: "Tu foto de perfil ha sido cambiada",
          type: "success",
        });
      }
    } catch (error) {
      showAlert({
        title: "Error",
        message: "No se pudo actualizar la foto de perfil. Inténtalo de nuevo.",
        type: "error",
      });
    }
  };
  ...
  const handleVerifiedPress = () => {
    showAlert({
      ...
      message: "Este usuario es miembro Premium de PlanMyRoute 🌟",
      type: "info",
    });
  };
  ...
  const listItems: (FollowerUser | FollowingUser)[] =
    followListType === "followers"
      ? (followersList ?? [])
      : (followingList ?? []);
  const listLoading =
    followListType === "followers" ? followersLoading : followingLoading;
          ...
          <View className="flex-row items-center justify-center mb-4">
            ...
            <View className="ml-4 flex-1">
              <View className="flex-row items-center gap-2">
                <Title2Semibold className="shrink">
                  {displayName}
                </Title2Semibold>
                {isVerified && (
                  <TouchableOpacity
              ...
              <View className="flex-row gap-3 mt-2">
                ...
                >
                  <TextRegular className="text-dark">
                    <TextRegular style={{ fontFamily: "Urbanist-SemiBold" }}>
          ...
          {!isOwnProfile && (
            <View className="mb-6">
              <CustomButton
                ...
                size="medium"
                loading={
                  isFollowPending || isUnfollowPending || isFollowingLoading
                }
            ...
            {listLoading ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => {
                  ...
                  const avatarUri =
                    u.img ||
                    `https://ui-avatars.com/api/?name=${u.username}&background=FFD54D&color=202020&size=80`;
                  return (
                    <TouchableOpacity
