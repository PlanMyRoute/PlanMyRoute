...
export function MapLocationPickerWeb({
  visible,
  initialLocation,
  onLocationSelect,
  onClose,
    ...
    const handleMessage = (event: MessageEvent) => {
      try {
        ...
        if (
          ...
          const newCoords: MapPickerCoords = {
            latitude: data.lat,
            longitude: data.lng,
          };
          setCoords(newCoords);
          ...
          debounceRef.current = setTimeout(async () => {
            try {
              const json = await apiFetch<{ address?: string }>(
                `/api/places/reverse?lat=${data.lat}&lng=${data.lng}`,
              );
