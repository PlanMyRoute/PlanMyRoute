...
export const DateTimePickerWeb: React.FC<DateTimePickerWebProps> = ({
  ...
  /** Formatea Date para inputs HTML de fecha o hora */
  ...
  return (
    <View className={containerClassName}>
      {label && <MicrotextDark className="mb-2">{label}</MicrotextDark>}
      <input
        ref={inputRef as any}
        type={mode === "date" ? "date" : "time"}
        value={formatDateForInput(value)}
        onChange={(e) => handleChange(e.target.value)}
        min={mode === "date" ? toISODate(minimumDate || null) : undefined}
        max={mode === "date" ? toISODate(maximumDate || null) : undefined}
        style={{
