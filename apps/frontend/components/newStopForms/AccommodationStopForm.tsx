...
function timeStringToDate(time: string | undefined, fallbackHour: number): Date {
    const date = new Date();
    if (time) {
...
function TimeField({
    ...
    return (
        <View className="flex-1">
            <Text className="text-xs font-semibold text-neutral-gray mb-1.5">{label}</Text>
            <TouchableOpacity
            ...
            >
                ...
                <Text className={`ml-2 text-sm ${value ? 'text-dark-black' : 'text-neutral-gray'}`}>
                    {value || placeholder}
                </Text>
            </TouchableOpacity>
            <CustomDateTimePicker
                ...
                onConfirm={(date) => {
                    setShowPicker(false);
                    onChange(`${date.getHours().toString().padStart(2, '0')}:${date.getMinutes().toString().padStart(2, '0')}`);
                }}
