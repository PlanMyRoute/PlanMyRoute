...
const CATEGORIES = [
    'Museo', 'Parque', 'Restaurante', 'Café', 'Monumento',
    'Playa', 'Montaña', 'Tienda', 'Teatro', 'Cine',
    'Galería', 'Mercado', 'Otro',
];

export default function ActivityStopForm({ activityData, onUpdateField }: ActivityStopFormProps) {
    ...
    useEffect(() => { setEntryPriceStr(activityData.entry_price?.toString() || ''); }, [activityData.entry_price]);
    useEffect(() => { setDurationStr(activityData.estimated_duration_minutes?.toString() || ''); }, [activityData.estimated_duration_minutes]);
    ...
    return (
        <View className="bg-gray-50 border border-neutral-gray/20 rounded-2xl p-4 gap-3">
            ...
            {/* Categoría */}
            <View>
                <Text className="text-xs font-semibold text-neutral-gray mb-1.5">Categoría</Text>
                <TouchableOpacity
                    onPress={() => setShowCategoryModal(true)}
                    className="bg-white border border-neutral-gray/20 rounded-xl px-3 py-3 flex-row items-center justify-between"
                >
                    <Text className={`text-sm ${activityData.category ? 'text-dark-black' : 'text-neutral-gray'}`}>
                    ...
                    <View className="flex-1 bg-black/50 justify-end">
                        <View className="bg-white rounded-t-3xl p-5">
                            <View className="flex-row items-center justify-between mb-4">
                                <Text className="text-base font-bold text-dark-black">Categoría</Text>
                            ...
                            <ScrollView className="max-h-96" showsVerticalScrollIndicator={false}>
                                {CATEGORIES.map((cat) => {
                                    ...
                                    return (
                                        <TouchableOpacity
                                            key={cat}
                                            onPress={() => { onUpdateField('category', cat); setShowCategoryModal(false); }}
                                        ...
                                        >
                                            <Text className={`font-semibold text-sm ${selected ? 'text-dark-black' : 'text-neutral-gray'}`}>
                                                {cat}
                                            </Text>
                                        </TouchableOpacity>
                                    );
            ...
            <View className="flex-row gap-3">
                <View className="flex-1">
                    <Text className="text-xs font-semibold text-neutral-gray mb-1.5">Precio entrada (€)</Text>
                    <TextInput
                        placeholder="0.00"
                ...
                <View className="flex-1">
                    <Text className="text-xs font-semibold text-neutral-gray mb-1.5">Duración (min)</Text>
                    <TextInput
                        placeholder="60"
                        className="bg-white border border-neutral-gray/20 rounded-xl px-3 py-2.5 text-sm text-dark-black"
                        placeholderTextColor="#9CA3AF"
            ...
            {/* Precio estimado */}
            <View>
                <Text className="text-xs font-semibold text-neutral-gray mb-1.5">Precio estimado (€)</Text>
                <TextInput
            ...
            {/* URL */}
            <View>
                <Text className="text-xs font-semibold text-neutral-gray mb-1.5">URL</Text>
                <TextInput
                    placeholder="https://..."
                    className="bg-white border border-neutral-gray/20 rounded-xl px-3 py-2.5 text-sm text-dark-black"
                    placeholderTextColor="#9CA3AF"
            ...
            <TouchableOpacity
                ...
                activeOpacity={0.7}
            >
                <View className={`w-5 h-5 rounded-md border-2 items-center justify-center ${activityData.booking_required ? 'bg-dark-black border-dark-black' : 'bg-white border-neutral-gray/40'}`}>
                    {activityData.booking_required && <Ionicons name="checkmark" size={13} color="#FFD54D" />}
                </View>
