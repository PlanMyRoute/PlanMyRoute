...
export const TripStatusCheck: React.FC<TripStatusCheckProps> = ({
  notification,
  userId,
}) => {
  ...
  const isStartNotification =
    notification.content?.toLowerCase().includes("comenzar") ||
  ...
  const handleConfirm = async () => {
    ...
    try {
      const result = await apiFetch<{ message: string }>(
        `/api/trip/${notification.related_trip_id}/status/respond`,
        {
          method: "POST",
      ...
      queryClient.invalidateQueries({
        queryKey: ["trip", notification.related_trip_id],
      });

      Toast.show({
        type: "success",
        text1: isStartNotification
          ? "¡Buen viaje! 🚗"
          : "¡Viaje completado! 🎉",
        text2: result.message,
        onPress: () => {
          Toast.hide();
          if (notification.related_trip_id != null) {
