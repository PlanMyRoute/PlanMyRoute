...
export function useStopPrice(
  ...
  const hasLocalPrice =
    localPriceInfo &&
    Object.values(localPriceInfo).some((v) => v !== undefined && v !== null);

  // Hook siempre se ejecuta (no puede estar después de un return condicional — Reglas de Hooks)
  const query = useQuery<StopPriceResponse, Error>({
    ...
    enabled: !!stopId && enabled && !hasLocalPrice,
