...
describe("useTripPermissions Hook", () => {
  it("Debe dar permisos totales si el usuario es OWNER", () => {
    ...
    expect(result.current.canRemoveTravelers).toBe(true); // La propiedad correcta es 'canRemoveTravelers'
  ...
  it("Debe dar todos los permisos si estamos creando un viaje nuevo (isCreating=true)", () => {
    ...
    expect(result.current.canEdit).toBe(true); // En modo creación, el usuario puede editar
    expect(result.current.isOwner).toBe(true);
  });
});
