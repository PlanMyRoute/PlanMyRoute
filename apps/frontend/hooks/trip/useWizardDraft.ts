...
export async function saveDraftAsync(userId: string, draft: WizardDraft): Promise<void> {
    await AsyncStorage.setItem(draftKey(userId), JSON.stringify(draft));
}

export async function loadDraftAsync(userId: string): Promise<WizardDraft | null> {
    try {
