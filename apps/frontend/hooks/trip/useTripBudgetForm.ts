...
export function useTripBudgetForm(validationResult: TripValidationResult | null, step: number) {
    ...
    useEffect(() => { setMinBudgetInput(minBudget.toString()); }, [minBudget]);
    useEffect(() => { setMaxBudgetInput(maxBudget.toString()); }, [maxBudget]);
    ...
    useEffect(() => {
        if (validationResult?.budgetEst && step === 5 && !budgetManuallyModified) {
            setMinBudget(validationResult.budgetEst.estimate_min);
            setMaxBudget(validationResult.budgetEst.estimate_max);
        }
    ...
    const getValueFromPosition = (position: number) => {
        ...
        const raw = (position / sliderWidth) * (sliderMaxValue - MIN_VALUE) + MIN_VALUE;
        return Math.max(MIN_VALUE, Math.min(sliderMaxValue, Math.round(raw / 10) * 10));
    };
    ...
    const minPanResponder = PanResponder.create({
        ...
        onPanResponderMove: (_, gestureState) => {
            const newValue = getValueFromPosition(getPositionFromValue(minBudget) + gestureState.dx);
    ...
    const maxPanResponder = PanResponder.create({
        onStartShouldSetPanResponder: () => true,
        onPanResponderMove: (_, gestureState) => {
            const newValue = getValueFromPosition(getPositionFromValue(maxBudget) + gestureState.dx);
    ...
    return {
        ...
        budgetManuallyModified, setBudgetManuallyModified,
