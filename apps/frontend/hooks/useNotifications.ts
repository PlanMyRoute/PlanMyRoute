...
/**
 * Hook para obtener las notificaciones del usuario, ordenadas por estado y fecha.
 * Las notificaciones pendientes se muestran primero, seguidas por las más recientes.
 * @param userId - ID del usuario receptor de las notificaciones.
 * @param options - Opciones de configuración: habilitación de la query.
 * @returns Objeto con las notificaciones, estado de carga, error y función de refetch.
 */
export function useNotifications(
  ...
  const query = useQuery<Notification[], Error>({
    ...
    queryFn: () => {
      ...
      return NotificationService.getByReceiverId(userId, {
    ...
    select: (data) => {
      ...
      return [...data].sort((a, b) => {
        ...
        if (a.action_status === "pending" && b.action_status !== "pending")
          return -1;
        if (a.action_status !== "pending" && b.action_status === "pending")
          return 1;
...
/**
 * Hook para obtener las notificaciones de un viaje específico.
 * Ordena las notificaciones con las pendientes primero y luego por fecha descendente.
 * @param tripId - ID del viaje del cual obtener las notificaciones.
 * @param options - Opciones de configuración: habilitación de la query.
 * @returns Objeto con las notificaciones del viaje, estado de carga, error y función de refetch.
 */
export function useNotificationsByTrip(
...
/**
 * Hook para obtener una notificación individual por su ID.
 * @param notificationId - ID de la notificación a obtener.
 * @param options - Opciones de configuración: habilitación de la query.
 * @returns Objeto con la notificación, estado de carga, error y función de refetch.
 */
export function useNotification(
  ...
  const query = useQuery<Notification, Error>({
    ...
    queryFn: () =>
      NotificationService.getById(notificationId as string | number),
    enabled,
    staleTime: NOTIFICATIONS_STALE_TIME,
    retry: 1,
  });
...
/**
 * Hook de mutación para aceptar invitaciones con optimistic update.
 * Actualiza el estado de la notificación a 'accepted' de forma optimista y revierte en caso de error.
 * @param userId - ID del usuario cuyas notificaciones se actualizarán en cache.
 * @returns Mutación de React Query para aceptar una invitación.
 */
...
/**
 * Hook de mutación para rechazar invitaciones con optimistic update.
 * Actualiza el estado de la notificación a 'rejected' de forma optimista y revierte en caso de error.
 * @param userId - ID del usuario cuyas notificaciones se actualizarán en cache.
 * @returns Mutación de React Query para rechazar una invitación.
 */
export function useDeclineInvitation(userId?: string | number) {
  ...
  return useMutation({
    mutationFn: async (notificationId: string | number) => {
      await NotificationService.declineInvitation(
        notificationId,
        token || undefined,
      );
    },
 ...
 * @param userId - ID del usuario cuyas notificaciones se invalidarán tras la mutación.
 * @returns Mutación de React Query para marcar como leída.
 */
 ...
 * @param userId - ID del usuario cuyas notificaciones se invalidarán tras la mutación.
 * @returns Mutación de React Query para eliminar una notificación.
 */
...
/**
 * Hook de mutación para crear una nueva notificación.
 * Invalida las notificaciones del receptor tras la creación exitosa.
 * @returns Mutación de React Query para crear una notificación.
 */
    ...
    onSuccess: (_data, variables) => {
      ...
      if (variables.notification.user_receiver_id) {
        queryClient.invalidateQueries({
          queryKey: [
            "notifications",
            String(variables.notification.user_receiver_id),
          ],
        });
      }
    },
