...
/**
 * Hook para gestionar los vehículos del usuario: carga, agregar, editar y eliminar.
 * Recarga la lista automáticamente cuando la pantalla obtiene el foco.
 * @param userId - ID del usuario propietario de los vehículos.
 * @returns Objeto con la lista de vehículos, estado de carga, funciones de gestión y límite máximo.
 */
export function useVehicles(userId: string | undefined) {
  ...
  const handleDeleteVehicle = useCallback(
    async (vehicle: Vehicle) => {
      ...
      try {
        await VehicleService.deleteVehicle(userId, vehicle.id.toString(), {
          token: token || undefined,
        });
        showAlert({
          title: "¡Eliminado!",
