...
/**
 * Hook para obtener y actualizar las preferencias del usuario (zona horaria y actualización automática de estado).
 * @param userId - ID del usuario
 * @returns Preferencias, estado de carga, función de actualización y estados de la mutación
 */
export function useUserPreferences(userId: string | undefined) {
  ...
  const mutation = useMutation({
    mutationFn: async (newPreferences: UpdatePreferencesPayload) => {
      ...
      return await UserService.updateUserPreferences(
        userId,
        newPreferences,
        token,
      );
