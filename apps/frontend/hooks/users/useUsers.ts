...
/**
 * Hook para obtener un usuario por ID o nombre de usuario.
 * @param userId - ID del usuario (opcional)
 * @param username - Nombre de usuario (opcional)
 * @param options - Opciones de configuración: token de autenticación y habilitación
 * @returns Datos del usuario, estado de carga, error y función de recarga
 */
...
/**
 * Hook para obtener el perfil completo de un usuario (datos, estadísticas, intereses y estado premium).
 * @param userId - ID del usuario (opcional)
 * @param username - Nombre de usuario (opcional, tiene prioridad sobre userId)
 * @param options - Opciones de configuración: token de autenticación y habilitación
 * @returns Perfil del usuario, estado de carga, error y función de recarga
 */
export function useProfile(
  ...
  const queryProfile = useQuery<UseProfileData, Error>({
    ...
    retry: (failureCount, error) =>
      failureCount < 2 && !error.message.includes("No identifier provided"),
  });
...
/**
 * Hook para buscar usuarios por nombre de usuario.
...
export function useSearchUsers(
  ...
  const querySearchUsers = useQuery<User[], Error>({
    ...
    queryFn: () =>
      UserService.searchUsersByUsername(trimmedQuery, {
        token: options?.token,
      }),
